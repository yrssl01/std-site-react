function StatsSection() {
  return (
    <section className="flex flex-col gap-5 w-full bg-[#4d4d4d] text-white px-20 pt-6 mt-20 pb-20">
      <div>
        <h2 className="text-2xl">Ключевые показатели</h2>
      </div>
      <div className="flex justify-center md:justify-around gap-8 items-center flex-wrap">
        <div className="flex flex-col justify-center items-center grow basis-0">
          <svg
            width="127px"
            height="127px"
            viewBox="0 0 24.00 24.00"
            xmlns="http://www.w3.org/2000/svg"
            fill="#ffffff"
            stroke="#ffffff"
            strokeWidth="0.00024000000000000003"
          >
            <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
            <g
              id="SVGRepo_tracerCarrier"
              strokeLinecap="round"
              strokeLinejoin="round"
            ></g>
            <g id="SVGRepo_iconCarrier">
              <path d="M11 1v19h4V1zm3 18h-2V2h2zm-9-9v10h4V10zm3 9H6v-8h2zm9-14v15h4V5zm3 14h-2V6h2zM2 22h21v1H1V1h1z"></path>
              <path fill="none" d="M0 0h24v24H0z"></path>
            </g>
          </svg>
          <h3 className="font-bold text-2xl mt-2">18</h3>
          <p className="text-lg text-center">лет на рынке</p>
        </div>
        <div className="flex flex-col justify-center items-center grow basis-0">
          <svg
            width="127px"
            height="127px"
            viewBox="0 0 24.00 24.00"
            xmlns="http://www.w3.org/2000/svg"
            fill="#ffffff"
            stroke="#ffffff"
            strokeWidth="0.00024000000000000003"
          >
            <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
            <g
              id="SVGRepo_tracerCarrier"
              strokeLinecap="round"
              strokeLinejoin="round"
            ></g>
            <g id="SVGRepo_iconCarrier">
              {' '}
              <g>
                {' '}
                <path fill="none" d="M0 0h24v24H0z"></path>{' '}
                <path
                  fillRule="nonzero"
                  d="M12 11a5 5 0 0 1 5 5v6h-2v-6a3 3 0 0 0-2.824-2.995L12 13a3 3 0 0 0-2.995 2.824L9 16v6H7v-6a5 5 0 0 1 5-5zm-6.5 3c.279 0 .55.033.81.094a5.947 5.947 0 0 0-.301 1.575L6 16v.086a1.492 1.492 0 0 0-.356-.08L5.5 16a1.5 1.5 0 0 0-1.493 1.356L4 17.5V22H2v-4.5A3.5 3.5 0 0 1 5.5 14zm13 0a3.5 3.5 0 0 1 3.5 3.5V22h-2v-4.5a1.5 1.5 0 0 0-1.356-1.493L18.5 16c-.175 0-.343.03-.5.085V16c0-.666-.108-1.306-.309-1.904.259-.063.53-.096.809-.096zm-13-6a2.5 2.5 0 1 1 0 5 2.5 2.5 0 0 1 0-5zm13 0a2.5 2.5 0 1 1 0 5 2.5 2.5 0 0 1 0-5zm-13 2a.5.5 0 1 0 0 1 .5.5 0 0 0 0-1zm13 0a.5.5 0 1 0 0 1 .5.5 0 0 0 0-1zM12 2a4 4 0 1 1 0 8 4 4 0 0 1 0-8zm0 2a2 2 0 1 0 0 4 2 2 0 0 0 0-4z"
                ></path>{' '}
              </g>{' '}
            </g>
          </svg>
          <h3 className="font-bold text-2xl mt-2">5</h3>
          <p className="text-lg text-center">компаний в составе группы</p>
        </div>
        <div className="flex flex-col justify-center items-center grow basis-0">
          <svg
            version="1.0"
            id="Layer_1"
            xmlns="http://www.w3.org/2000/svg"
            xmlnsXlink="http://www.w3.org/1999/xlink"
            width="127px"
            height="127px"
            viewBox="0 0 64.00 64.00"
            enableBackground="new 0 0 64 64"
            xmlSpace="preserve"
            fill="#ffffff"
            stroke="#ffffff"
            strokeWidth="0.00064"
          >
            <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
            <g
              id="SVGRepo_tracerCarrier"
              strokeLinecap="round"
              strokeLinejoin="round"
            ></g>
            <g id="SVGRepo_iconCarrier">
              {' '}
              <g>
                {' '}
                <path
                  fill="#ffffff"
                  d="M18,10v54h9V53c0-0.553,0.447-1,1-1h8c0.553,0,1,0.447,1,1v11h9V20V4c0-2.211-1.789-4-4-4H22 c-2.211,0-4,1.789-4,4V10z M34,9c0-0.553,0.447-1,1-1h4c0.553,0,1,0.447,1,1v4c0,0.553-0.447,1-1,1h-4c-0.553,0-1-0.447-1-1V9z M34,19c0-0.553,0.447-1,1-1h4c0.553,0,1,0.447,1,1v4c0,0.553-0.447,1-1,1h-4c-0.553,0-1-0.447-1-1V19z M34,29c0-0.553,0.447-1,1-1 h4c0.553,0,1,0.447,1,1v4c0,0.553-0.447,1-1,1h-4c-0.553,0-1-0.447-1-1V29z M34,39c0-0.553,0.447-1,1-1h4c0.553,0,1,0.447,1,1v4 c0,0.553-0.447,1-1,1h-4c-0.553,0-1-0.447-1-1V39z M24,9c0-0.553,0.447-1,1-1h4c0.553,0,1,0.447,1,1v4c0,0.553-0.447,1-1,1h-4 c-0.553,0-1-0.447-1-1V9z M24,19c0-0.553,0.447-1,1-1h4c0.553,0,1,0.447,1,1v4c0,0.553-0.447,1-1,1h-4c-0.553,0-1-0.447-1-1V19z M24,29c0-0.553,0.447-1,1-1h4c0.553,0,1,0.447,1,1v4c0,0.553-0.447,1-1,1h-4c-0.553,0-1-0.447-1-1V29z M24,39c0-0.553,0.447-1,1-1 h4c0.553,0,1,0.447,1,1v4c0,0.553-0.447,1-1,1h-4c-0.553,0-1-0.447-1-1V39z"
                ></path>{' '}
                <rect x="26" y="30" fill="#ffffff" width="2" height="2"></rect>{' '}
                <rect x="26" y="40" fill="#ffffff" width="2" height="2"></rect>{' '}
                <rect x="29" y="54" fill="#ffffff" width="6" height="10"></rect>{' '}
                <rect x="8" y="50" fill="#ffffff" width="2" height="2"></rect>{' '}
                <rect x="26" y="10" fill="#ffffff" width="2" height="2"></rect>{' '}
                <rect x="26" y="20" fill="#ffffff" width="2" height="2"></rect>{' '}
                <rect x="36" y="20" fill="#ffffff" width="2" height="2"></rect>{' '}
                <rect x="36" y="10" fill="#ffffff" width="2" height="2"></rect>{' '}
                <rect x="36" y="30" fill="#ffffff" width="2" height="2"></rect>{' '}
                <rect x="36" y="40" fill="#ffffff" width="2" height="2"></rect>{' '}
                <rect x="8" y="40" fill="#ffffff" width="2" height="2"></rect>{' '}
                <path
                  fill="#ffffff"
                  d="M16,10H4c-2.211,0-4,1.789-4,4v46c0,2.211,1.789,4,4,4h12V10z M12,53c0,0.553-0.447,1-1,1H7 c-0.553,0-1-0.447-1-1v-4c0-0.553,0.447-1,1-1h4c0.553,0,1,0.447,1,1V53z M12,43c0,0.553-0.447,1-1,1H7c-0.553,0-1-0.447-1-1v-4 c0-0.553,0.447-1,1-1h4c0.553,0,1,0.447,1,1V43z M12,33c0,0.553-0.447,1-1,1H7c-0.553,0-1-0.447-1-1v-4c0-0.553,0.447-1,1-1h4 c0.553,0,1,0.447,1,1V33z M11,24H7c-0.553,0-1-0.447-1-1v-4c0-0.553,0.447-1,1-1h4c0.553,0,1,0.447,1,1v4 C12,23.553,11.553,24,11,24z"
                ></path>{' '}
                <rect x="54" y="50" fill="#ffffff" width="2" height="2"></rect>{' '}
                <rect x="54" y="40" fill="#ffffff" width="2" height="2"></rect>{' '}
                <rect x="54" y="30" fill="#ffffff" width="2" height="2"></rect>{' '}
                <path
                  fill="#ffffff"
                  d="M60,20H48v44h12c2.211,0,4-1.789,4-4V24C64,21.789,62.211,20,60,20z M58,53c0,0.553-0.447,1-1,1h-4 c-0.553,0-1-0.447-1-1v-4c0-0.553,0.447-1,1-1h4c0.553,0,1,0.447,1,1V53z M58,43c0,0.553-0.447,1-1,1h-4c-0.553,0-1-0.447-1-1v-4 c0-0.553,0.447-1,1-1h4c0.553,0,1,0.447,1,1V43z M58,33c0,0.553-0.447,1-1,1h-4c-0.553,0-1-0.447-1-1v-4c0-0.553,0.447-1,1-1h4 c0.553,0,1,0.447,1,1V33z"
                ></path>{' '}
                <rect x="8" y="30" fill="#ffffff" width="2" height="2"></rect>{' '}
                <rect x="8" y="20" fill="#ffffff" width="2" height="2"></rect>{' '}
              </g>{' '}
            </g>
          </svg>
          <h3 className="text-2xl font-bold mt-2">398</h3>
          <p className="text-lg text-center">объектов в прошли через нас</p>
        </div>
        <div className="flex flex-col justify-center items-center grow basis-0">
          <svg
            width="127px"
            height="127px"
            viewBox="0 0 128.00 128.00"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            stroke="#ffffff"
            strokeWidth="0.00128"
          >
            <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
            <g
              id="SVGRepo_tracerCarrier"
              strokeLinecap="round"
              strokeLinejoin="round"
            ></g>
            <g id="SVGRepo_iconCarrier">
              {' '}
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M37.0409 66.6549L60.4468 75.4433C62.2696 76.1278 64.2795 76.1239 66.0997 75.4325L88.962 66.7484L88.9618 83.4477C88.962 83.4635 88.9621 83.4792 88.9621 83.495C88.9621 88.6399 77.3391 92.8106 63.0015 92.8106C48.6638 92.8106 37.0409 88.6399 37.0409 83.495C37.0409 83.4898 37.0408 83.4845 37.0408 83.4792C37.0408 83.4687 37.0407 83.4582 37.0409 83.4477V66.6549Z"
                fill="#ffffff"
              ></path>{' '}
              <path
                d="M110.065 79.2054C110.846 77.7714 111.729 75.9086 112.477 74.2526C113.059 72.9632 114.716 72.6791 115.428 73.9011C116.677 76.0409 117.5 78.594 117.909 80.2952C118.039 80.8355 117.925 81.4215 117.509 81.7908C115.748 83.3571 113.028 82.9374 110.916 82.1365C109.783 81.7069 109.485 80.2694 110.065 79.2054Z"
                fill="white"
              ></path>{' '}
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M114.04 74.1257C113.817 74.1564 113.543 74.3211 113.388 74.664C112.637 76.3276 111.742 78.2169 110.943 79.6836C110.774 79.994 110.737 80.3575 110.815 80.6544C110.89 80.938 111.052 81.1184 111.271 81.2014C112.259 81.5763 113.345 81.8434 114.352 81.8514C115.352 81.8593 116.204 81.6137 116.845 81.0435C116.917 80.9798 117.004 80.8086 116.937 80.5293C116.54 78.8819 115.746 76.4307 114.565 74.405C114.422 74.1595 114.233 74.0991 114.04 74.1257ZM113.767 72.1445C114.726 72.0122 115.723 72.4207 116.292 73.3972L115.428 73.9011M113.767 72.1445C112.838 72.2726 111.993 72.8947 111.565 73.8412L113.767 72.1445ZM116.292 73.3972C117.607 75.6511 118.459 78.3061 118.881 80.0611C119.074 80.8625 118.933 81.8632 118.174 82.5381C117.054 83.5342 115.664 83.8619 114.336 83.8513C113.015 83.8408 111.685 83.4976 110.561 83.0715C108.685 82.3599 108.416 80.1426 109.186 78.7271C109.95 77.3259 110.821 75.4898 111.565 73.8412"
                fill="#ffffff"
              ></path>{' '}
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M112.252 71.6091C112.679 71.2594 113.309 71.3224 113.659 71.7498C114.002 72.169 114.178 72.785 114.289 73.2976C114.41 73.863 114.494 74.524 114.552 75.1886C114.668 76.521 114.688 77.963 114.671 78.8885C114.661 79.4407 114.205 79.8802 113.653 79.8702C113.101 79.8602 112.661 79.4044 112.672 78.8522C112.687 77.9757 112.668 76.6066 112.56 75.3621C112.505 74.7382 112.431 74.1694 112.334 73.7189C112.285 73.4938 112.235 73.3157 112.186 73.1838C112.14 73.0593 112.109 73.0142 112.109 73.0139C112.109 73.0138 112.11 73.0147 112.111 73.0163C111.761 72.5889 111.824 71.9589 112.252 71.6091Z"
                fill="#ffffff"
              ></path>{' '}
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M113.475 52.3058C114.303 52.3058 114.975 52.9774 114.975 53.8058V70.4171C114.975 71.2456 114.303 71.9171 113.475 71.9171C112.646 71.9171 111.975 71.2456 111.975 70.4171V53.8058C111.975 52.9774 112.646 52.3058 113.475 52.3058Z"
                fill="#ffffff"
              ></path>{' '}
              <path
                d="M117.603 71.0069C117.603 73.2869 115.755 75.1352 113.475 75.1352C111.195 75.1352 109.347 73.2869 109.347 71.0069C109.347 68.727 111.195 66.8787 113.475 66.8787C115.755 66.8787 117.603 68.727 117.603 71.0069Z"
                fill="#ffffff"
              ></path>{' '}
              <path
                d="M60.1547 33.0997L11.6849 52.4133C10.7891 52.7703 10.8019 54.0427 11.7046 54.3817L60.3131 72.6331C62.2367 73.3554 64.3578 73.3513 66.2786 72.6217L114.302 54.3803C115.2 54.0393 115.212 52.774 114.322 52.4149L66.4363 33.1122C64.422 32.3003 62.1722 32.2958 60.1547 33.0997Z"
                fill="#ffffff"
              ></path>{' '}
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M100.016 55.5271C100.205 56.0459 99.938 56.62 99.4192 56.8093L65.2136 69.2924C64.6947 69.4817 64.1207 69.2146 63.9313 68.6958C63.742 68.177 64.0091 67.6029 64.5279 67.4136L98.7335 54.9305C99.2524 54.7412 99.8264 55.0083 100.016 55.5271Z"
                fill="white"
              ></path>{' '}
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M19.5669 53.2122C19.7571 52.6937 20.3317 52.4275 20.8502 52.6177L32.2819 56.8112C32.8004 57.0014 33.0665 57.5759 32.8763 58.0944C32.6861 58.6129 32.1116 58.8791 31.5931 58.6889L20.1614 54.4954C19.6429 54.3052 19.3767 53.7307 19.5669 53.2122Z"
                fill="white"
              ></path>{' '}
            </g>
          </svg>
          <h3 className="font-bold text-2xl mt-2">1</h3>
          <p className="text-lg text-center">категория лицензий</p>
        </div>
      </div>
    </section>
  )
}

export default StatsSection
